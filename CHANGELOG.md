...
### 🚀 **Performance Improvements**
  ...
  - **Parallel ZIP extraction** — When 7-Zip is available and multiple ZIPs are present, up to 2 archives are now extracted concurrently. The per-process thread count is halved (`-mmt=N/2`) so total CPU usage stays the same, while disk I/O throughput is better utilised on SSD-class storage. Native Dart extraction remains sequential to avoid simultaneous heap pressure from two large ZIPs.
